...
The CG loop tracks two monotone bounds. `best_ub` is the minimum LP objective over iterations whose LP primal is MCF-feasible (no slack basic, no new capacity row found this iter). `best_lb` is the maximum Lagrangian/Farley lower bound over the same MCF-feasible iters when the pricer visited every source (`pricer.priced_all()` — no source postponed; a `max_cols` break that fires exactly on sweep completion still counts): `LB = dual_obj + Σ_k d_k · min(rc*_k, 0) − rounding_margin`, where `dual_obj = Σ π·RHS + Σ cap·μ` (computed from the captured duals, not the LP primal, to avoid barrier-interior primal/dual drift), `rc*_k` is the pricer's best reduced cost for entity k, and the rounding margin bounds the scale-integer vs true-RC gap in A*'s chosen path. Path formulation weights by `d_k` (demand); tree formulation's convexity row has RHS=1 so the weighting collapses. Early exit fires when `best_ub − best_lb < RELATIVE_FEAS_TOL · max(1, |best_ub|)` and the gap is non-negative. The iteration log's `#slk` column is the count of basic slack columns — when non-zero the LP obj is a feasibility penalty, not a bound, and neither UB nor LB is updated that iter.
...
- **Use the tree formulation.** `solve_tree_cg` is robust on BUS/SBT under both HiGHS and COPT, both PricerLight and PricerHeavy — all four combinations converge to the paper's LP optimum on BUS-2632. The path formulation, however, stalls indefinitely under HiGHS (slacks stay basic, LP stops improving around iter ~400 with UB=inf) and is substantially slower under COPT. Integration tests for intermodal use `solve_tree_cg` + `PricerHeavy` via the `solve_intermodal_and_check` helper.
