...
class HiGHSSolver : public LPSolver {
   ...
   public:
    ...
    uint32_t add_cols(const std::vector<double> & obj,
        ...
        _highs.addCols(n, obj.data(), lb.data(), ub.data(), 0, nullptr, nullptr,
                       nullptr);
    ...
    uint32_t add_cols(const std::vector<double> & obj,
        ...
        _highs.addCols(n, obj.data(), lb.data(), ub.data(), nnz,
                       h_starts.data(), h_indices.data(), values.data());
        _num_cols += static_cast<uint32_t>(n);
        return first;
    }
    ...
    uint32_t add_rows(const std::vector<double> & lb,
        ...
        _highs.addRows(m, lb.data(), ub.data(), nnz, h_starts.data(),
                       h_indices.data(), values.data());
