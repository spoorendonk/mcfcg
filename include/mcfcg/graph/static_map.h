...
public:
    ...
    constexpr explicit static_map(size_type size)
        : _data(std::make_unique_for_overwrite<mapped_type[]>(size)), _size(size) {}

    constexpr static_map(size_type size, const mapped_type &init) : static_map(size) {
    ...
    constexpr explicit static_map(R &&r) : static_map(std::ranges::begin(r), std::ranges::end(r)) {}

    static_map(const static_map &other) : static_map(other.data(), other.data() + other.size()) {}
