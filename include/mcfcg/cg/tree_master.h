...
private:
    ...
    std::vector<TreeColumn> _columns;
    std::vector<uint32_t> _col_to_lp;
...
public:
    ...
    void init(const Instance& inst, std::unique_ptr<LPSolver> lp = nullptr) {
        ...
        std::vector<double> slack_lb(_num_source_rows, 0.0);
        std::vector<double> slack_ub(_num_source_rows, INF);
        _lp->add_cols(slack_obj, slack_lb, slack_ub);

        // Source convexity rows: sum_tau lambda^s_tau + slack_s = 1
    ...
    uint32_t add_columns(std::vector<TreeColumn> cols) {
        ...
        for (auto& col : cols) {
            ...
            // Convexity row coefficient
            row_indices.push_back(col.source_idx);
            values.push_back(1.0);
    ...
    std::vector<double> get_source_duals() const {
        auto all = _lp->get_duals();
        return std::vector<double>(all.begin(), all.begin() + _num_source_rows);
    }
    ...
    uint32_t purge_nonbinding_capacity_rows(uint32_t current_iter, uint32_t inactivity_threshold) {
        ...
        for (uint32_t i = 0; i < _cap_row_to_arc.size(); ++i) {
            uint32_t old_row = _num_source_rows + i;
