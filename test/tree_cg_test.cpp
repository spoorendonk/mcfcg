...
static void verify_path_tree_agreement(const std::string & path,
                                       double expected_obj) {
    ...
    EXPECT_NEAR(path_result.objective, expected_obj, 1e-4)
        << "path CG wrong objective";
    EXPECT_NEAR(tree_result.objective, expected_obj, 1e-4)
        << "tree CG wrong objective";
    EXPECT_NEAR(path_result.objective, tree_result.objective, 1e-4)
        << "path and tree disagree";
}
